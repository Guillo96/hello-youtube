...
class Fetcher(webapp.RequestHandler):
  ...
  def __init__(self):
    ...
    self.my_app_domain = 'http://localhost:8080'
  ...
  def get(self):
    ...
    self.response.out.write('<li><a href="%s">YouTube API</a></li>' % (
        self.client.GenerateAuthSubURL(
            self.my_app_domain + '/authsub' + 
                '?token_scope=' + self.youtube_scope, 
  ...
  def FetchFeed(self):
    ...
    if not self.feed_url:
      ...
      self.response.out.write('Here\'s an example query which will show the' +
          ...
          self.my_app_domain + '/authsub?' + 
          'feed_url=' + self.feed_url + '</a>')
      return
    try:
      feed = self.client.GetYouTubeVideoFeed(self.feed_url)

      self.response.out.write('My videos:<ul>')
